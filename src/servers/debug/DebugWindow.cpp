...
DebugWindow::DebugWindow(const char* appName)
	:
	BWindow(BRect(0, 0, 100, 100), "Crashed program", B_MODAL_WINDOW,
	...
	fAction(kActionKillTeam)
{
	BString buffer(B_TRANSLATE(
		"The application:\n\n      %app\n\n"
		"has encountered an error which prevents it from continuing. FegeyaOS "
	...
	fOKButton = new BButton("close", B_TRANSLATE("Close"),
	...
	switch(message->what) {
		...
		case kActionSaveReportTeam:
			...
			fOKButton->SetLabel(fAction == kActionDebugTeam
				? B_TRANSLATE("Nice") : B_TRANSLATE("Close"));
			return;
	}
