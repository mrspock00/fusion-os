...
MainWindow::_AddDefaultButtons()
{
	...
	fPadView->AddButton(button);
	button->SetTo("application/x-vnd.Haiku-Appearance", true);

	//Backgrounds
	
	button = new LaunchButton("launch button", NULL, new BMessage(MSG_LAUNCH));
	fPadView->AddButton(button);
	button->SetTo("application/x-vnd.Haiku-Backgrounds", true);

	// Keyboard 
	button = new LaunchButton("launch button", NULL, new BMessage(MSG_LAUNCH));
	fPadView->AddButton(button);
	button->SetTo("application/x-vnd.Haiku-Keyboard", true);

	// Mouse 
	
	button = new LaunchButton("launch button", NULL, new BMessage(MSG_LAUNCH));
	fPadView->AddButton(button);
	button->SetTo("application/x-vnd.Haiku-Mouse", true);	
	
	// FegeyaBrowser (WebPositive)
